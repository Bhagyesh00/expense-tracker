...
export class GroqProvider {
  ...
  constructor(apiKey: string) {
    ...
    this.rateLimit = { timestamps: [], maxPerMinute: 30 };
  }
  ...
  async categorizeExpense(
    ...
    const prompt = `Given an expense description, classify it into one of the provided categories.
...
{"categoryId": "<id>", "confidence": <0.0-1.0>, "reasoning": "<brief explanation>"}`;
    ...
    return this.generateText(prompt, {
      temperature: 0.1,
      systemPrompt: 'You are an expense categorization assistant. Respond only with valid JSON.',
    });
  }

  async generateInsight(context: string, question: string): Promise<string> {
    const prompt = `Based on the user's financial data, answer their question concisely and helpfully.
...
Provide a clear, actionable answer. Include specific numbers when relevant. Keep the response under 200 words.`;
    ...
    return this.generateText(prompt, {
      temperature: 0.4,
      systemPrompt: 'You are a personal finance insights assistant.',
    });
  }

  async generateAlert(anomalyData: string): Promise<string> {
    const prompt = `Generate a brief, conversational alert message for the following spending anomaly.
...
{"title": "<short title>", "body": "<conversational message, 1-2 sentences>", "emoji": "<single relevant emoji>", "actionSuggestion": "<what the user can do>"}`;
    ...
    return this.generateText(prompt, {
      temperature: 0.5,
      systemPrompt: 'You are a friendly financial alert assistant. Respond only with valid JSON.',
    });
  }

  async generateForecastNarrative(forecastData: string): Promise<string> {
    const prompt = `Generate a brief narrative summary of the user's spending forecast for the rest of the month.
